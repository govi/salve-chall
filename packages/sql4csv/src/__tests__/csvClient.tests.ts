...
describe("csv client tests", () => {
  it("should read csv content and return an object", async () => {
    ...
    const result = await client.query<{ id: string; name: string }>("clinics")
  ...
  it("should throw when requesting tenant owned items without an tenant-id", async () => {
    const client = new CSVClient(defaultOptions)
    const result = await client.query<{ id: string; date_of_birth: string }>(
      "patients",
      ["date_of_birth"],
      { clinic_id: 2 }
    )
    ...
    expect(result[2].date_of_birth).toBe("1983-11-23")
  })
})
